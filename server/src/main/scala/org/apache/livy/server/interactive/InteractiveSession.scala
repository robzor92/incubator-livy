...
object InteractiveSession extends Logging {
  ...
  private[interactive] def prepareBuilderProp(
    ...
    val sparkPyFiles = conf.get(LivyConf.SPARK_PY_FILES).map(_.split(",")).getOrElse(Array.empty[String])
    mergeConfList(sparkPyFiles, LivyConf.SPARK_PY_FILES)

    val sparkJars = conf.get(LivyConf.SPARK_YARN_DIST_JARS).map(_.split(",")).getOrElse(Array.empty[String])
    mergeConfList(sparkJars, LivyConf.SPARK_YARN_DIST_JARS)
